...
let switch
  ...
  let learn c_id pi =
    ...
    let tbl', present = match ClientTable.find tbl c_id with
      | None -> (ClientTable.add tbl c_id (EthTable.singleton eth port), false)
      | Some e_tbl ->
        match EthTable.find e_tbl eth with
          | None -> (ClientTable.add tbl c_id (EthTable.add e_tbl eth port), false)
  ...
  let forward (tbl : switchTable) c_id t_id pi =
    ...
    let out =
      match EthTable.find (ClientTable.find_exn tbl c_id) dst with
        | None -> Flood
  ...
  begin match evt with
    | `SwitchUp (c_id, feats) ->
      ...
      >>= ensure (ClientTable.add tbl c_id EthTable.empty)
    | `SwitchDown c_id -> return (ClientTable.remove tbl c_id)
    | `Message (c_id, msg) ->
      let t_id, msg = msg in
      begin match msg with
        ...
        | PortStatusMsg ps ->
          ...
          begin match ps.reason with
            | ChangeReason.Delete ->
              let tbl' = ClientTable.change tbl c_id (function
                  | None -> None
                  | Some eth_tbl ->
...
let main () =
  ...
  create 6633 ()
  >>= fun t ->
    Pipe.fold (listen t) ~init:ClientTable.empty ~f:(switch t)
